...
After each fold finishes training, this helper reloads the fold's best
checkpoint, runs inference on that fold's held-out calibration split, and saves
``q05``, ``q95``, and ground-truth ``Cd`` values. ``merge_cvplus_oof.py`` later
merges these files into one asymmetric CQR calibration value.
"""
