...
def _restore_config_from_checkpoint(config: Config, checkpoint: dict) -> None:
    ...
    for key in (
        ...
        if key in ckpt_cfg:
            setattr(config, key, ckpt_cfg[key])
...
def evaluate_model(model: torch.nn.Module, dataloader, device: torch.device):
    ...
    with torch.no_grad():
        for point_clouds, global_geometry_descriptors, cd_values in dataloader:
            ...
            pred = model(point_clouds, global_geometry_descriptors).detach().cpu().numpy()
            for idx, q in enumerate(quantiles):
                predictions[q].extend(pred[:, idx].tolist())
            targets.extend(cd_values.detach().cpu().numpy().reshape(-1).tolist())

    predictions = {q: np.asarray(v, dtype=np.float64) for q, v in predictions.items()}
    return predictions, np.asarray(targets, dtype=np.float64)
...
def calculate_metrics(predictions: dict[float, np.ndarray], y: np.ndarray) -> dict[str, float]:
    q05 = predictions[0.05]
    q50 = predictions[0.5]
    q95 = predictions[0.95]
    ...
    corr = float(np.corrcoef(q50, y)[0, 1]) if y.size > 1 else float("nan")
...
def _load_cqr_hat_q(path: str) -> tuple[float, float, int | None, str]:
    ...
    return (
        ...
        float(payload["q_u"]),
        payload.get("n_cal"),
        payload.get("protocol", "precomputed_cqr"),
    )
...
def _calibrate_cqr(model, config: TestConfig, device: torch.device) -> tuple[float, float, int, str]:
    ...
    if cal_loader is None:
        raise FileNotFoundError(
            "Calibration CSV is missing. Set CALIBRATION_CSV or provide CQR_HAT_Q_JSON."
        )
    pred_norm, y_norm = evaluate_model(model, cal_loader, device)
    y_cal = _denormalize(config, y_norm)
    q05_cal = _denormalize(config, pred_norm[0.05])
    q95_cal = _denormalize(config, pred_norm[0.95])
    q_l, q_u = asymmetric_cqr_hat_q(q05_cal, q95_cal, y_cal, alpha=config.CQR_ALPHA)
    return q_l, q_u, int(y_cal.size), "single_calibration_split"
...
def main() -> None:
    ...
    logger.info("Loaded model from epoch %s", checkpoint.get("epoch", "unknown"))

    q_l, q_u, n_cal, calibration_protocol = _calibrate_cqr(model, config, device)
    ...
    metrics.update(
        {
            ...
            "CQR_q_u": float(q_u),
            "CQR_n_cal": int(n_cal),
            "CQR_protocol": calibration_protocol,
        }
    )

    design_ids = _read_design_ids(config.TEST_CSV, config.DESIGN_COLUMN, y.size)
    out_df = pd.DataFrame(
        {
    ...
    predictions_csv = Path(config.RESULTS_DIR) / "predictions.csv"
    intervals_csv = Path(config.RESULTS_DIR) / "per_sample_cd_cp_intervals.csv"
    metrics_json = Path(config.RESULTS_DIR) / "metrics.json"
    metrics_txt = Path(config.RESULTS_DIR) / "metrics.txt"

    out_df.to_csv(predictions_csv, index=False, encoding="utf-8")
    out_df.to_csv(intervals_csv, index=False, encoding="utf-8")
    ...
    logger.info("Saved predictions: %s", predictions_csv)
