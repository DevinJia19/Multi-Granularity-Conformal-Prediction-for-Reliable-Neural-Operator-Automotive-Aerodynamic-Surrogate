...
# 若已有缓存可跳过推理（仅占位 GPU 时间短），但仍需能加�?torch 等依赖�?#
...
#SBATCH --output=/cephyr/users/chaoxi/Alvis/Desktop/mimer_naiss2025-6-430/ChundongJia/Transolver_Cd_CP_without_cvplus/logs/cp_replicates_%j.out
#SBATCH --error=/cephyr/users/chaoxi/Alvis/Desktop/mimer_naiss2025-6-430/ChundongJia/Transolver_Cd_CP_without_cvplus/logs/cp_replicates_%j.err
...
# 可选：数据路径�?evaluate_cp_replicates.py / Config 一致（�?checkpoint 里已有则不必设）
...
CACHE_NPZ="${CP_EVAL_CACHE:-${PROJECT_ROOT}/results/cp_eval_cache.npz}"
OUT_JSON="${CP_EVAL_OUT_JSON:-${PROJECT_ROOT}/results/cp_replicates_summary.json}"
