...
class DataAugmentation:
    ...
    def jitter_pointcloud(pointcloud: torch.Tensor, sigma: float = 0.01, clip: float = 0.02) -> torch.Tensor:
        ...
        jittered_pointcloud = pointcloud.clone()
        jittered_pointcloud[:, :3] = pointcloud[:, :3] + torch.clamp(
            sigma * torch.randn(N, 3), -clip, clip
        )
...
class DrivAerNetDataset(Dataset):
    ...
    def __init__(
        ...
        self.surface_features = bool(surface_features)
        self.use_curvature = bool(use_curvature)
        if self.use_curvature and not self.surface_features:
            raise ValueError("use_curvature=True 需要 surface_features=True")
    ...
    @staticmethod
    def point_feature_dim(surface_features: bool = True, use_curvature: bool = False) -> int:
        dim = 3
        if surface_features:
            dim += 4  # nx, ny, nz, area_weight
        if use_curvature:
            dim += 1
        return dim
    ...
    def normalize_pointcloud(self, data: torch.Tensor) -> torch.Tensor:
        """
        Center xyz coordinates without per-sample scaling.
        Surface attributes (normals / area / curvature) are left unchanged.
        """
        if data.shape[-1] <= 3:
            return data - data.mean(dim=0, keepdim=True)
        out = data.clone()
        out[..., :3] = out[..., :3] - out[..., :3].mean(dim=0, keepdim=True)
        return out

    @staticmethod
    def compute_vertex_curvature_proxy(vertices: np.ndarray, faces: np.ndarray) -> np.ndarray:
        """Per-vertex mean-curvature proxy via uniform-graph Laplacian magnitude."""
        vertices = np.asarray(vertices, dtype=np.float64)
        faces = np.asarray(faces, dtype=np.int64)
        n_verts = len(vertices)
        neighbor_sums = np.zeros((n_verts, 3), dtype=np.float64)
        neighbor_counts = np.zeros(n_verts, dtype=np.float64)
        for tri in faces:
            for a, b, c in ((0, 1, 2), (1, 2, 0), (2, 0, 1)):
                i, j, k = int(tri[a]), int(tri[b]), int(tri[c])
                neighbor_sums[i] += vertices[j] + vertices[k]
                neighbor_counts[i] += 2.0
        valid = neighbor_counts > 0.0
        laplacian = np.zeros(n_verts, dtype=np.float64)
        laplacian[valid] = np.linalg.norm(
            neighbor_sums[valid] / neighbor_counts[valid, None] - vertices[valid],
            axis=1,
        )
        scale = float(np.median(laplacian[laplacian > 0.0])) if np.any(laplacian > 0.0) else 1.0
        return (laplacian / (scale + 1e-8)).astype(np.float32)
        ...
        if vertices is None or global_geometry_descriptors is None:
            ...
            if mesh_payload is None:
                ...
                if self.use_curvature:
                    mesh_payload["vertex_curvatures"] = self.compute_vertex_curvature_proxy(
            ...
            vertices = self._sample_surface_points_from_arrays(
                ...
                vertex_curvatures=mesh_payload.get("vertex_curvatures"),
            )
            if self.enable_point_cache:
    ...
    def _cache_file_path(self, design_id: str) -> str:
        key = (
            ...
            f"surf={int(self.surface_features)}|curv={int(self.use_curvature)}"
        )
    ...
    def _load_cached_mesh_payload(self, design_id: str):
        ...
        try:
            ...
            mesh_payload = {
                ...
                "faces": payload["faces"].astype(np.int64) if "faces" in payload else None,
                "triangles": payload["triangles"].astype(np.float32),
                "area_faces": payload["area_faces"].astype(np.float64),
                "vertex_curvatures": (
                    payload["vertex_curvatures"].astype(np.float32)
                    if "vertex_curvatures" in payload
                    else None
                ),
            }
            if self.surface_features and mesh_payload["faces"] is None:
                return None
            if self.use_curvature and mesh_payload["vertex_curvatures"] is None:
                return None
            return mesh_payload
        except Exception as e:
    ...
    def _save_cached_mesh_payload(self, design_id: str, mesh_payload) -> None:
        ...
        try:
            ...
            if mesh_payload.get("faces") is not None:
                payload_to_save["faces"] = np.asarray(mesh_payload["faces"], dtype=np.int64)
            if mesh_payload.get("vertex_curvatures") is not None:
                payload_to_save["vertex_curvatures"] = np.asarray(
                    mesh_payload["vertex_curvatures"], dtype=np.float32
                )
            np.savez_compressed(cache_path, **payload_to_save)
        except Exception as e:
    ...
    def _sample_surface_points_from_arrays(
        ...
        vertex_curvatures: Optional[np.ndarray] = None,
    ) -> torch.Tensor:
        if rng is None:
            rng = np.random.default_rng()
        try:
            ...
            normals = (normals / np.clip(normal_norm, 1e-8, None)).astype(np.float32)
            area_weights = (area_faces[face_indices] / area_sum).astype(np.float32)[:, None]

            feature_parts = [sampled_points, normals, area_weights]
            if self.use_curvature:
                if vertex_curvatures is None or faces is None:
                    raise ValueError("use_curvature=True 需要 mesh faces 与 vertex_curvatures")
                face_vertices = np.asarray(faces, dtype=np.int64)[face_indices]
                vertex_curvatures = np.asarray(vertex_curvatures, dtype=np.float32)
                curv = (
                    w0 * vertex_curvatures[face_vertices[:, 0]]
                    + w1 * vertex_curvatures[face_vertices[:, 1]]
                    + w2 * vertex_curvatures[face_vertices[:, 2]]
                ).astype(np.float32)[:, None]
                feature_parts.append(curv)

            sampled_features = np.concatenate(feature_parts, axis=-1).astype(np.float32)
            return torch.from_numpy(sampled_features)
        except Exception as e:
            ...
            raise ValueError(
                "Surface-feature sampling failed and vertex-only fallback is disabled "
                "when surface_features=True"
            )
