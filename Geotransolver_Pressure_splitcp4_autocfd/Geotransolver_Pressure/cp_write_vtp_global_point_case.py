...
Do NOT pass results/pressure_cvplus_cp/q_hat.json from cp_calibrate_cvplus_normalized.py;
that file only contains point_sigma-style scalars. Use qhat.json from
cp_compare_global_point_case.py (keys: global_abs, point_sigma, case_sigma).

Example (CV+ calib glob + final test npz):
    python cp_compare_global_point_case.py \
      --calib-glob "runs/cvplus/fold_*/cvplus_oof/fold_*/*.npz" \
      --test-dir runs/final_autocfd/cp_pointwise_test_final \
      --out results/cp_compare_global_point_case_cvplus

    python cp_write_vtp_global_point_case.py \
      --test-dir runs/final_autocfd/cp_pointwise_test_final \
      --qhat-json results/cp_compare_global_point_case_cvplus/qhat.json \
      --out results/cp_compare_global_point_case_cvplus/vtp \
...
def load_qhats(qhat_json: Path) -> Dict[str, np.ndarray]:
    ...
    if not out:
        legacy = obj.get("q_hat", obj)
        if isinstance(legacy, dict) and all(ch in legacy for ch in CHANNELS):
            raise ValueError(
                f"{qhat_json} looks like point-only output from "
                "cp_calibrate_cvplus_normalized.py (top-level q_hat with pressure/wss_*). "
                "Run cp_compare_global_point_case.py first and pass its qhat.json, which "
                f"contains modes: {MODES}."
            )
