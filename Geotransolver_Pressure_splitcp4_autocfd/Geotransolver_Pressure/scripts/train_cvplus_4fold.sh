#!/usr/bin/env bash
#SBATCH --job-name=pressure-cvplus4
...
#SBATCH --output=logs/pressure_cvplus4_%j.out
#SBATCH --error=logs/pressure_cvplus4_%j.err
...
mkdir -p "${PROJECT_ROOT}/runs/cvplus_oof_pressure"
...
for FOLD in 0 1 2 3; do
  ...
  bash "${PROJECT_ROOT}/scripts/test.sh" \
    "run_id=${FOLD_RUN_ID}" \
    "checkpoint_dir=${PROJECT_ROOT}/runs/${FOLD_RUN_ID}/checkpoints_best" \
    "inference.phase=val" \
    "data.val.data_path=data_splits/calib" \
    "cp_output.save_pointwise_npz=true" \
    "cp_output.subdir=cvplus_oof/fold_${FOLD}" \
    "data.resolution=8192"
done

mkdir -p "${PROJECT_ROOT}/results/pressure_cvplus_cp"
...
echo "[INFO] CV+ q_hat written to ${PROJECT_ROOT}/results/pressure_cvplus_cp/q_hat.json"
echo "[INFO] CV+ folds finished."
