...
def deterministic_metrics_1d(pred: np.ndarray, target: np.ndarray) -> dict[str, float]:
    ...
    if n < 2:
        ...
        denom = float(
            np.sqrt(np.sum(pred_centered * pred_centered) * np.sum(target_centered * target_centered))
        )
        pearson = float(np.sum(pred_centered * target_centered) / denom) if denom > 0.0 else float("nan")
...
def read_vehicle_map(path: Path | None) -> dict[str, dict[str, str]]:
    ...
    The existing split-CP helper writes columns like fold_0_batch_file and
...
def summarize_case_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    channels = sorted({str(r["channel"]) for r in rows}, key=lambda x: CHANNELS.index(x) if x in CHANNELS else x)
    ...
    for label in labels:
        for ch in channels:
            ...
            out: dict[str, Any] = {
                ...
                "channel": ch,
                "n_cases": len(subset),
                "mean_n_points": float(np.mean([float(r["n_points"]) for r in subset])) if subset else float("nan"),
            }
...
def main() -> int:
    ...
    ap.add_argument("--test-glob", type=str, action="append", default=None,
                    help="Glob matching test .npz files or directories. Can be repeated.")
    ap.add_argument("--splitcp4-runs-root", type=Path, default=None,
                    help="Shortcut for runs/splitcp_4fold/fold_i/splitcp_test/fold_i.")
    ap.add_argument("--n-folds", type=int, default=4)
    ap.add_argument("--vehicle-map", type=Path, default=None,
    ...
    payload = {
        ...
        "metrics": METRICS,
        "datasets": [
            {"label": label, "n_cases": len(files), "path_examples": [str(p) for p in files[:3]]}
            for label, files in datasets
        ],
