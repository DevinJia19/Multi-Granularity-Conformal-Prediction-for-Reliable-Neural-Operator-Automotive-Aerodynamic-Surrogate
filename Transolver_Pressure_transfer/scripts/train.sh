...
SPLIT_N_SPLITS="${SPLIT_N_SPLITS:-4}"
SPLIT_CALIB_FOLD="${SPLIT_CALIB_FOLD:-0}"

RUN_SPLIT="${RUN_SPLIT:-1}"
if [[ "${RUN_SPLIT}" == "1" ]]; then
    SPLIT_ARGS=(
        --n-splits "${SPLIT_N_SPLITS}"
        --calib-fold "${SPLIT_CALIB_FOLD}"
        --symlink
    )
...
# ----- Step 3: plain Transolver training (train.py + conf/config.yaml) -----
# 默认 4×A100；单机多卡可覆盖：NPROC_PER_NODE=1 torchrun ...
...
EXTRA_HYDRA_ARGS=(
    ...
    "training.validate_every=${VALIDATE_EVERY:-1}"
...
echo "[INFO] data.resolution=8192"
echo "[INFO] SPLIT_N_SPLITS=${SPLIT_N_SPLITS}"
echo "[INFO] SPLIT_CALIB_FOLD=${SPLIT_CALIB_FOLD}"
echo "[INFO] VALIDATE_EVERY=${VALIDATE_EVERY:-1}"
echo "[INFO] EARLY_STOPPING_PATIENCE=${EARLY_STOPPING_PATIENCE:-0}"
