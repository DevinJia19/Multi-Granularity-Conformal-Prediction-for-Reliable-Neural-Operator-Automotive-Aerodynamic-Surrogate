#!/usr/bin/env python3
"""Split AutoCFD DrivAerML surface zarr runs for single-run Transolver CP."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "with one calibration holdout inside the official train pool."
        )
    )
    ...
    parser.add_argument(
        ...
        default=4,
        help=(
            "Number of calibration splits inside official train. Default 4 gives "
            "300 train + 100 calibration from the 400 official training cases."
        ),
    )
    parser.add_argument(
        "--calib-fold",
        type=int,
        default=0,
        help="Calibration split index used as the holdout (0 .. n_splits-1).",
    )
    parser.add_argument(
        "--final-train",
        action="store_true",
...
def main():
    ...
    if args.final_train:
        ...
        fold_train_ids, fold_calib_ids = calibration_holdout_split(
            AUTO_CFD_TRAIN_IDS, args.n_splits, args.calib_fold
        )
        train_paths = _resolve_paths(fold_train_ids, by_id, "single CP train")
        calib_paths = _resolve_paths(fold_calib_ids, by_id, "single CP calibration")
    ...
    if args.final_train:
        ...
        print("current calibration holdout: 0 (disabled with --final-train)")
    else:
        print(f"current split train: {len(train_paths)}")
        print(f"current split calibration: {len(calib_paths)}")
        print(f"single CP n_splits={args.n_splits}, calib_fold={args.calib_fold}")
    ...
    meta = {
        "protocol": "autocfd_official_split_single_cp",
