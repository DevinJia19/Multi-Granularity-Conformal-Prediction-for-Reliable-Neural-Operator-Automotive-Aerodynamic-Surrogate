...
"""Combined training: Transolver backbone + uncertainty-aware drag head.
...
from train import (
    ...
    global_embedding_for_transolver_cp,
    maybe_center_geometry_with_stl_com,
    prepare_global_features_for_transolver_cp,
...
def compute_transolver_drag_full_mesh(
    ...
    """Run Transolver on full mesh in chunks and integrate to Cd.
...
def main(cfg: DictConfig):
    """Run combined Transolver + GP/MLP training loop."""
    ...
    # consistency_detach_transolver: if true, stop gradients through Transolver
    ...
    # embedding_feat_dim: input feature dimension from Transolver context
    ...
    # ---- Transolver (DDP-wrapped, trainable) ----
    ...
    logger.info(f"Transolver parameters: {num_geo_params:,}")
    ...
    sync_use_fourier_datapipe_config(train_dl, cfg)
    sync_use_fourier_datapipe_config(val_dl, cfg)
    if val_dl_full is not None:
        sync_use_fourier_datapipe_config(val_dl_full, cfg)
    ...
    for epoch in range(loaded_epoch, cfg.training.num_epochs):
        ...
        for i, batch in enumerate(train_dl):
            ...
            with get_autocast_context(precision):
                ...
                center_geo = getattr(cfg.data, "center_geometry_with_stl_com", True)
            ...
            if embedding_states is None:
                raise RuntimeError(
                    ...
                    "Combined training requires a Transolver model that returns embedding_states."
                )
