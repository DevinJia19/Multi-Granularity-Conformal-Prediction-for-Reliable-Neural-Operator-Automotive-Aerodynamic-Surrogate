...
"""
Inference script for running trained Transolver models on raw VTK files.
...
Usage (surface inference with Transolver):
    python inference_on_vtk.py --config-name=config \
...
Usage (volume 或 Transolver 等变体)：在统一配置 ``config`` 上通过 Hydra 覆盖 ``model`` / ``data.mode`` 等键即可，例如：
    python inference_on_vtk.py --config-name=config \
        data.mode=volume \
        +vtk_inference.input_dir=/path/to/runs \
        +vtk_inference.output_dir=/path/to/output
...
from train import sync_use_fourier_datapipe_config, update_model_params_for_fp8
...
def inference_on_vtk(cfg: DictConfig) -> None:
    ...
    sync_use_fourier_datapipe_config(datapipe, cfg)
...
def launch(cfg: DictConfig) -> None:
    ...
    Uses existing Transolver configs. VTK-specific parameters
