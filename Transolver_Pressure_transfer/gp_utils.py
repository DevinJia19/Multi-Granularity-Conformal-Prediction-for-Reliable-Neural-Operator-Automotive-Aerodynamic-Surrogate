...
"""Utilities for the Transolver + GP drag prediction pipeline.
...
def compute_drag_target_from_batch(
    ...
    area_key = "surface_areas" if "surface_areas" in batch else "surface_areas_sub"
    normals = batch[normal_key].squeeze(0).to(device, dtype=fields_phys.dtype)
    area = batch[area_key].squeeze(0).to(device, dtype=fields_phys.dtype)
...
def compute_drag_from_subsampled_outputs(
    ...
    """Monte-Carlo drag estimate from subsampled Transolver predictions.

    Preserves the computational graph through *outputs* so gradients can
    flow back into the Transolver. Returns ``(1,)`` in GP-scaled space.
    """
    ...
    if "surface_areas" in batch:
        ...
        n_full = batch["surface_areas_sub"].squeeze(0).shape[0]
    n_sub = p.shape[0]
    coeff = 2.0 / (FRONTAL_AREA * REFERENCE_DENSITY * REFERENCE_VELOCITY**2)
    scale = n_full / n_sub
