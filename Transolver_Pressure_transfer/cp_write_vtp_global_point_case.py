...
Input .npz files are expected to contain:
    pred, target, surface_mesh_centers
Optional:
    sigma, surface_normals, surface_areas
...
Use qhat.json from cp_compare_global_point_case.py
(keys: global_abs, point_sigma, case_sigma).

Example (single-run Transolver CP):
    ...
    if not out:
        ...
        if isinstance(legacy, dict) and all(ch in legacy for ch in CHANNELS):
            raise ValueError(
                ...
                "a point-only q_hat file (top-level q_hat with pressure/wss_*). "
