...
def global_embedding_for_transolver_cp(
    features: torch.Tensor,
    embeddings: torch.Tensor,
) -> torch.Tensor:
...
def prepare_global_features_for_transolver_cp(
    features: torch.Tensor,
    embeddings: torch.Tensor,
    datapipe: TransolverDataPipe,
) -> torch.Tensor:
    """Optional normalize then collapse broadcast global to ``(B, 1, 6)``."""
    g = features
    ...
    return global_embedding_for_transolver_cp(g, embeddings)
...
def split_mean_sigma(outputs: Any) -> tuple[torch.Tensor, torch.Tensor | None]:
    ...
    the training / inference code compatible with plain Transolver outputs.
    """
...
def forward_pass(
    ...
    Otherwise uses Transolver ``global_embedding`` / ``local_embedding`` /
    ...
    with get_autocast_context(precision):
        if legacy_transolver_forward:
            ...
            global_features = prepare_global_features_for_transolver_cp(
...
def val_epoch(
    ...
    local = torch.tensor(
        [float(total_loss), float(n_val_steps)],
    ...
    global_total_loss = float(local[0].item())
    global_count = max(float(local[1].item()), 1.0)
    ...
    if total_metrics:
        for k, v in total_metrics.items():
            metric_sum = torch.tensor(
                [float(v.detach().sum().item() if hasattr(v, "detach") else float(v))],
                device=dist_manager.device,
                dtype=torch.float64,
            )
            if dist_manager.world_size > 1:
                dist.all_reduce(metric_sum, op=dist.ReduceOp.SUM)
            avg_metrics[k] = metric_sum.item() / global_count
...
def main(cfg: DictConfig):
    ...
    logger.info(f"Surface sampling resolution: {cfg.data.resolution}")
    logger.info(
        f"overfit_deterministic_subsample: "
        f"{getattr(cfg.data, 'overfit_deterministic_subsample', False)}"
    )
    logger.info(
        f"overfit_fixed_train_batch: "
        f"{getattr(cfg.data, 'overfit_fixed_train_batch', False)}"
    )
    logger.info(
        "Training uses stochastic 8192-point surface sampling unless "
        "overfit deterministic mode is enabled."
    )
    ...
    if getattr(cfg.training, "use_legacy_transolver_forward", False):
        ...
        logger.info(
            ...
            "Transolver global_embedding / local_embedding / local_positions "
    ...
    early_stopping_patience = int(
        getattr(cfg.training, "early_stopping_patience", 0)
    )
    if dist_manager.rank == 0:
        logger.info(f"training.validate_every: {validate_every}")
        logger.info(
            f"training.early_stopping_patience: {early_stopping_patience} "
            f"({'disabled' if early_stopping_patience <= 0 else 'enabled'})"
        )
    ...
    if not skip_validation:
        ...
        logger.info(
            ...
            "Keep data_splits/calib for CP calibration only."
        )
    ...
    best_ckpt_args = dict(ckpt_args)
    best_ckpt_args["path"] = f"{checkpoint_dir}/{cfg.run_id}/checkpoints_best"
    ...
    for epoch in range(loaded_epoch, cfg.training.num_epochs):
        ...
        if do_validation and not np.isnan(val_loss):
        ...
        if improved:
            ...
            if dist_manager.rank == 0:
                ...
                logger.info(
                    f"New best {metric_name}={best_metric:.6f} at epoch {epoch + 1}; "
                    f"saved to {best_ckpt_args['path']}"
                )
        else:
    ...
    if dist_manager.rank == 0:
        if best_epoch >= 0:
            logger.info(
                f"Training completed. Best epoch {best_epoch + 1} "
                f"with metric={best_metric:.6f} -> {best_ckpt_args['path']}"
            )
        else:
